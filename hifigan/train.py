...
from dataset import MelDataset, get_dataset_filelist, extract_features
...
def train(rank, args, hp, hp_str):
    ...
    optim_g = torch.optim.AdamW(generator.parameters(), hp.train.adamG.lr, betas=[hp.train.adamG.beta1,
                                                                                  hp.train.adamG.beta2])
    ...
    trainset = MelDataset(args.audio_format, training_filelist, hp.audio.segment_length, hp.audio.filter_length,
                          hp.audio.hop_length, hp.audio.win_length, hp.audio.sampling_rate, hp.data.input_wavs,
                          hp.data.output_wavs, hp.audio.mel_fmin, hp.audio.mel_fmax, True, False, n_cache_reuse=0,
                          fmax_loss=hp.audio.fmax_for_loss, device=device, fine_tuning=args.fine_tuning,
                          base_mels_path=hp.data.mel_path, features=args.features, num_mels=hp.audio.n_mel_channels,
                          center=hp.audio.center, min_amp=hp.audio.min_amp)
    ...
    if rank == 0:
        validset = MelDataset(args.audio_format, validation_filelist, hp.audio.segment_length, hp.audio.filter_length,
                              hp.audio.hop_length, hp.audio.win_length, hp.audio.sampling_rate,  hp.data.input_wavs,
                              hp.data.output_wavs, hp.audio.mel_fmin, hp.audio.mel_fmax, False, False, n_cache_reuse=0,
                              fmax_loss=hp.audio.fmax_for_loss, device=device, fine_tuning=args.fine_tuning,
                              base_mels_path=hp.data.mel_path, features=args.features, num_mels=hp.audio.n_mel_channels,
                              center=hp.audio.center, min_amp=hp.audio.min_amp)
        ...
        for i, batch in enumerate(train_loader):
            ...
            if y_g_hat is not None:
                ...
                for audio in y_g_hat.squeeze(1).detach().cpu().numpy():
                    single_mel = extract_features(args.features, audio, hp.audio.sampling_rate, hp.audio.win_length,
                                                  hp.audio.hop_length, hp.audio.filter_length,
                                                  hp.audio.mel_fmin, hp.audio.fmax_for_loss, hp.audio.n_mel_channels,
                                                  hp.audio.center, hp.audio.min_amp)
            ...
            print(np.shape(y_mel_loss))
            print(np.shape(before_y_g_hat))
            ...
            before_y_g_hat_mel = []
            for audio in before_y_g_hat.squeeze(1).detach().cpu().numpy():
                single_mel = extract_features(args.features, audio, hp.audio.sampling_rate, hp.audio.win_length,
                                              hp.audio.hop_length, hp.audio.filter_length,
                                              hp.audio.mel_fmin, hp.audio.fmax_for_loss, hp.audio.n_mel_channels,
                                              hp.audio.center, hp.audio.min_amp)
                before_y_g_hat_mel.append(single_mel)
            before_y_g_hat_mel = np.swapaxes(before_y_g_hat_mel, 1, 2)
            # pull to gpu
            before_y_g_hat_mel = torch.tensor(before_y_g_hat_mel)
            print(np.shape(before_y_g_hat_mel))
            before_y_g_hat_mel = before_y_g_hat_mel.to("cuda:0")
            # Generator
            optim_g.zero_grad()
                ...
                if steps % hp.logs.validation_interval == 0:  # and steps != 0:
                    ...
                    with torch.no_grad():
                        for j, batch in enumerate(validation_loader):
                            ...
                            y_g_hat_mel = []
                            for audio in before_y_g_hat.squeeze(1).detach().cpu().numpy():
                                single_mel = extract_features(args.features, audio, hp.audio.sampling_rate,
                                                              hp.audio.win_length,
                                                              hp.audio.hop_length, hp.audio.filter_length,
                                                              hp.audio.mel_fmin, hp.audio.fmax_for_loss,
                                                              hp.audio.n_mel_channels, hp.audio.center, hp.audio.min_amp)
                                y_g_hat_mel.append(single_mel)
                            ...
                            if j <= 4:
                                ...
                                sw.add_audio('generated/y_hat_{}'.format(j), before_y_g_hat[0], steps,
                                ...
                                for audio in before_y_g_hat.squeeze(1).detach().cpu().numpy():
                                    single_mel = extract_features(args.features, audio, hp.audio.sampling_rate,
                                                                  hp.audio.win_length, hp.audio.hop_length,
                                                                  hp.audio.filter_length, hp.audio.mel_fmin,
                                                                  hp.audio.fmax_for_loss, hp.audio.n_mel_channels,
                                                                  hp.audio.center, hp.audio.min_amp)
                                    y_hat_spec.append(single_mel)
                                y_hat_spec = torch.tensor(y_hat_spec)
                                y_hat_spec = np.swapaxes(y_hat_spec, 1, 2)
                                print(np.shape(y_hat_spec))
                                y_hat_spec = y_hat_spec.to("cuda:0")
...
def main():
    ...
    parser.add_argument('--train_file', default=None, help='path to LJSpeech training file')
    parser.add_argument('--valid_file', default=None, help='path to LJSpeech validation  file')
    ...
    parser.add_argument('--audio_format', default='.ogg', help="choose target audio representation to load the "
